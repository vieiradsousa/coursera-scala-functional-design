...
trait Encoder[-A] {
  ...
  def encode(value: A): Json
    ...
    */
  def transform[B](f: B => A): Encoder[B] = Encoder.fromFunction[B](value => this.encode(f(value)))
}
...
trait EncoderInstances {
  ...
  implicit val intEncoder: Encoder[Int] = Encoder.fromFunction(int => Json.Num(BigDecimal(int)))
  ...
  implicit val booleanEncoder: Encoder[Boolean] = Encoder.fromFunction(bool => Json.Bool(bool))
...
trait Decoder[+A] {
  ...
  def zip[B](that: Decoder[B]): Decoder[(A, B)] =
    Decoder.fromFunction {
      json => this.decode(json).zip(that.decode(json))
    }
...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(its) => {
        val itsDecoded = its.map(decoder.decode)

        if (itsDecoded.forall(_.isDefined))
          Some(itsDecoded.map(_.get))
        else
          None
      }
      case _ => None
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform[Contacts](person => person.people)

  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform(p => Contacts(people = p))
