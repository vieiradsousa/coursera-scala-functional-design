...
class BloxorzSuite {
  ...
  @Test def `the most recent move is the head of the list`: Unit =
    new Level1 {
      val inputBlock = Block(Pos(1, 1), Pos(1, 1))
      val inputHistory = List(Left, Up)

      val outputLazyList = Set(
        (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ).to(LazyList)

      assertEquals(outputLazyList, neighborsWithHistory(inputBlock, inputHistory))
    }

  @Test def `not get lost in circles of movements`: Unit =
    new Level1 {
      val neighbors = Set(
        (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ...
      assertEquals(outputLazyList, newNeighborsOnly(neighbors, explored))
    }
